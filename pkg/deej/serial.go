...
// GetCurrentSliderValues returns the current normalized values for all sliders
func (sio *SerialIO) GetCurrentSliderValues() []float32 {
	if !sio.connected || len(sio.currentSliderPercentValues) == 0 {
		return nil
	}

	// Return a copy to avoid race conditions
	result := make([]float32, len(sio.currentSliderPercentValues))
	copy(result, sio.currentSliderPercentValues)
	return result
}
